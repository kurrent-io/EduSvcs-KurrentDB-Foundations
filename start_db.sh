...
db_version=kurrent-latest/kurrentdb:latest
...
echo "Starting KurrentDB docker container. This can take a moment..."
...
#####################################################################################
#
# Step 2. Stop and remove the KurrentDB docker container if it is already running
#
#####################################################################################

if docker inspect esdb-node > /dev/null 2>&1; then                            # If the KurrentDB docker container (esdb-node) is running
       echo -e 'KurrentDB docker container appears to be running...';
       echo -e 'Stopping and removing the KurrentDB docker container...';
...
################################################################################################################################################
#
# Step 3. Run the KurrentDB docker container
#
...
#      -p 2113:2113 \               # Map port 2113 on the host to port 2113 in the docker container. Required for KurrentDB
#      eventstore/eventstore:lts \  # Specify the Docker image to use, in this case, the KurrentDB long-term support version (lts)
...
echo -e 'Starting the KurrentDB docker container...';
...
if ! docker inspect esdb-node > /dev/null 2>&1; then    # If the KurrentDB docker container (esdb) is running, exit
       echo "The KurrentDB docker container is not running. Exiting."
       exit 1
fi

######################################################################################
#
# Step 4. Print success message if KurrentDB docker container started successfully
#
######################################################################################
